...
import { cancelWaitingTimer, cancelPermissionTimer, clearAgentActivity } from '../src/timerManager.js';
import { JSONL_POLL_INTERVAL_MS, PROJECT_SCAN_INTERVAL_MS } from '../src/constants.js';
import { createAgentPane, focusAgentPane, killAgentPane, isTmuxPaneAlive } from './tmuxLauncher.js';
import { readPersistedAgents, writePersistedAgents, readSettings } from './settingsPersistence.js';
...
export function launchNewAgent(ctx: StandaloneContext): number {
	...
	console.log(`[Standalone] Agent ${id}: created (pane=${pane.paneId}, session=${pane.sessionId})`);
	...
	const pollTimer = setInterval(() => {
		try {
			if (fs.existsSync(agent.jsonlFile)) {
				console.log(`[Standalone] Agent ${id}: found JSONL file ${path.basename(agent.jsonlFile)}`);
				clearInterval(pollTimer);
				ctx.jsonlPollTimers.delete(id);
				startFileWatching(
					id, agent.jsonlFile, ctx.agents,
					ctx.fileWatchers, ctx.pollingTimers, ctx.waitingTimers,
					ctx.permissionTimers, ctx.webview as never,
				);
				readNewLines(id, ctx.agents, ctx.waitingTimers, ctx.permissionTimers, ctx.webview as never);
			}
...
export function restoreAgents(ctx: StandaloneContext): void {
	...
	for (const p of persisted) {
		...
		if (!isTmuxPaneAlive(p.tmuxPaneId)) {
			console.log(`[Standalone] Agent ${p.id}: pane ${p.tmuxPaneId} is dead, skipping`);
			continue;
		}
		...
		try {
			if (fs.existsSync(p.jsonlFile)) {
				...
				startFileWatching(
					p.id, p.jsonlFile, ctx.agents,
					ctx.fileWatchers, ctx.pollingTimers, ctx.waitingTimers,
					ctx.permissionTimers, ctx.webview as never,
				);
				console.log(`[Standalone] Restored agent ${p.id} (pane=${p.tmuxPaneId})`);
			}
...
export function startProjectScan(ctx: StandaloneContext): void {
	...
	// Seed known files
	try {
		const files = fs.readdirSync(ctx.projectDir)
...
function scanForNewJsonlFiles(ctx: StandaloneContext): void {
	let files: string[];
	try {
		files = fs.readdirSync(ctx.projectDir)
			.filter((f: string) => f.endsWith('.jsonl'))
			.map((f: string) => path.join(ctx.projectDir, f));
	} catch { return; }
	...
	for (const file of files) {
		if (!ctx.knownJsonlFiles.has(file)) {
			...
			console.log(`[Standalone] New JSONL file detected: ${path.basename(file)}`);
...
function autoAdoptJsonlFile(ctx: StandaloneContext, jsonlFile: string): void {
	...
	console.log(`[Standalone] Agent ${id}: auto-adopted ${path.basename(jsonlFile)}`);
	ctx.webview?.postMessage({ type: 'agentCreated', id });

	startFileWatching(
		id, jsonlFile, ctx.agents,
		ctx.fileWatchers, ctx.pollingTimers, ctx.waitingTimers,
		ctx.permissionTimers, ctx.webview as never,
	);
	readNewLines(id, ctx.agents, ctx.waitingTimers, ctx.permissionTimers, ctx.webview as never);
